...
// Tests that initializing DipNet with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		runDipNet(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		dipnet := runDipNet(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		dipnet.ExpectRegexp(tt.result)
		dipnet.ExpectExit()
	}
}
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			dipnet := runDipNet(t, args...)
			dipnet.ExpectRegexp(tt.initExpect)
			dipnet.ExpectExit()
		}
		{ // Exec + query
			...
			dipnet := runDipNet(t, args...)
			dipnet.ExpectRegexp(tt.execExpect)
			dipnet.ExpectExit()
		}
		return nil
	}
	for i, tt := range []backendTest{
		...
		{ // Can't start pebble on top of leveldb
			initArgs:   []string{"--db.engine", "leveldb"},
			execArgs:   []string{"--db.engine", "pebble"},
			execExpect: `Fatal: Failed to register the DipNet service: db.engine choice was pebble but found pre-existing leveldb database in specified data directory`,
		},
		{ // Can't start leveldb on top of pebble
			initArgs:   []string{"--db.engine", "pebble"},
			execArgs:   []string{"--db.engine", "leveldb"},
			execExpect: `Fatal: Failed to register the DipNet service: db.engine choice was leveldb but found pre-existing pebble database in specified data directory`,
		},

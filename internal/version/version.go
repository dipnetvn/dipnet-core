...
import (
	...
	"github.com/dipnetvn/dipnet-core/version"
)

const ourPath = "github.com/dipnetvn/dipnet-core" // Path to our module
...
// Archive holds the textual version string used for DipNet archives. e.g.
...
// conventions in the DipNet p2p network.
...
//
// If the package that is currently executing is a prefixed by our dipnet-core
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "dipnet".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("dipnet %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("dipnet %s", WithMeta)
	}

...
// Package accounts implements high level DipNet account management.
package accounts
...
// Account represents an DipNet account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // DipNet account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain dipnet.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19DipNet Signed Message:\n%d%s", len(data), data)

...
import (
	...
	"github.com/dipnetvn/dipnet-core/accounts/usbwallet/trezor"
...
// or whether the DipNet app was not started on it.
...
// DipNet address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.DipNetAddress)
	if _, err := w.trezorExchange(&trezor.DipNetGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.DipNetTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.DipNetTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}

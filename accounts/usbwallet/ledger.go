...
// https://github.com/LedgerHQ/app-dipnet/blob/develop/doc/ethapp.adoc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and DipNet address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an DipNet transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an DipNet message following the EIP 712 specification
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "DipNet app in browser mode", w.failure
	}
	if w.offline() {
		return "DipNet app offline", w.failure
	}
	return fmt.Sprintf("DipNet app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the DipNet app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// DipNet app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the DipNet app's version, will fail prior to v1.0.2
...
// and returning the DipNet address located on that derivation path.
...
//
// Note, if the version of the DipNet application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the DipNet app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the DipNet wallet app running
...
// ledgerDerive retrieves the currently active DipNet address from a Ledger
...
//	DipNet address length | 1 byte
//	DipNet address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the DipNet hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex string into an DipNet address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// https://github.com/LedgerHQ/app-dipnet/issues/409
	chunk := 255
	...
	// Extract the DipNet signature and do a sanity validation

...
// the process. This is usually the case when DipNet is shutting down and some events
...
// data on disk when two subchains are joined todipneter.
...
//
// Opposed to the original DipNet block synchronization which is trustless (and
...
// not from within (vs. legacy DipNet sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but DipNet is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until DipNet requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altodipneter

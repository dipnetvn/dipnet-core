...
import (
	...
	"github.com/dipnetvn/dipnet-core/eth/fetcher"
...
// support all the operations needed by the DipNet chain protocols.
...
// newHandler returns a handler for all DipNet chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	peer.Log().Debug("DipNet peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("DipNet peer registration failed", "err", err)
		return err
	}
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Warn("DipNet peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing DipNet peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("DipNet peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("DipNet protocol stopped")
}

...
// Package filters implements an dipnet filtering system for block,
...
func (es *EventSystem) SubscribeLogs(crit dipnet.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeLogs(crit dipnet.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
